...
export function parseAttendanceCsv(text, { knownUsernames } = {}) {
  const rows = parseRows(String(text || '').replace(/^\uFEFF/, '')).filter((row) =>
  ...
  if (usernameIndex < 0 || presentIndex < 0 || dateIndex < 0)
    throw new TypeError('CSV must include username, present, and date columns');
  ...
  rows.slice(1).forEach((row, offset) => {
    ...
    if (!username) rejected.push({ row: rowNumber, username: '', reason: 'Username is required' });
      ...
      rejected.push({ row: rowNumber, username, reason: 'Present must be 1, 0, true, or false' });
    else if (knownUsernames && !knownUsernames.has(username))
      rejected.push({ row: rowNumber, username, reason: 'Unknown username' });
    else if (!/^\d{4}-\d{2}-\d{2}$/.test(rowDate))
      rejected.push({ row: rowNumber, username, reason: 'Date must be YYYY-MM-DD' });
    else if (date && rowDate !== date)
      rejected.push({ row: rowNumber, username, reason: 'All rows must use the same date' });
    else {
      date ||= rowDate;
