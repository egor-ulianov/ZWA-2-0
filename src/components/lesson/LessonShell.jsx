...
export function useSlideNavigation(slides) {
  ...
  const setActiveSlide = useCallback((requestedSlide) => {
    const nextSlide = slides.some((slide) => slide.id === requestedSlide) ? requestedSlide : fallback;
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-zinc-50 to-sky-50 dark:from-zinc-950 dark:to-zinc-900 text-zinc-900 dark:text-zinc-50">
      <div className={`${maxWidthClass} mx-auto p-4 md:p-8 relative`}>
        ...
        <header className="mb-6">
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2">{title || lesson.title}</h1>
          {subtitle && <p className="text-sm text-zinc-500">{subtitle}</p>}
        </header>

        <SlideNavigation slides={slides} activeSlide={activeSlide} onChange={onChange} idPrefix={idPrefix} />
