...
function validateCssBasics({ slideId, inspection, htmlCode, cssCode }) {
  ...
  const hasVisitedRule = /footer\s+a\s*:\s*visited[\s\S]*?color\s*:\s*#?2563eb/i.test(
    cssCode
  );
  results.push({
    ok: Boolean(
  ...
  const hasFirstLetterSize =
    /::\s*first-letter[\s\S]*font-size\s*:\s*200%/i.test(cssCode);
  const hasFirstLetterBackground =
    /::\s*first-letter[\s\S]*background\s*:\s*#?fef08a/i.test(cssCode);
  results.push({
  ...
  results.push({
    ok: Boolean(
      submenu &&
        ["lower-alpha", "lower-alpha outside"].includes(
          submenu["list-style-type"]
        )
    ),
...
function validateCssLayout({ stepIndex, inspection, htmlCode, cssCode }) {
  ...
  if (stepIndex === 0) {
    ...
    results.push({
      ok:
        /clear\s*:\s*both/i.test(cssCode) ||
        /::after[\s\S]*clear\s*:\s*both/i.test(cssCode),
    ...
    results.push({
      ok: Boolean(
        highlight && ["inline-block", "block"].includes(highlight.display)
      ),
    ...
    results.push({
      ok: /<link[^>]*rel=["']stylesheet["'][^>]*media=["']print["'][^>]*>/i.test(
        htmlCode
      ),
