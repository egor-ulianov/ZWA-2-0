...
<footer style="margin-top: 16px; padding-top: 12px; border-top: 1px solid #e5e7eb;">
  ...
  useEffect(() => {
    ...
    const html = `<style>${cssCode}</style>${htmlTemplate}`;
    previewRef.current.innerHTML = html;
  }, [cssCode, htmlTemplate, applyVersion]);
  ...
  function validate() {
    ...
    try {
      ...
      pass.push({ ok: !!(okColor && okSize), text: "Task 1: h1 is blue and 36px" });
    } catch (_) {
      pass.push({ ok: false, text: "Task 1: Error checking styles" });
    }

    // Task 2: footer links font and color (visited same)
    try {
      ...
      // For :visited equivalence we approximate by checking code presence
      const hasVisitedRule = /footer\s+a\s*:\s*visited[\s\S]*?color\s*:\s*#?2563eb/i.test(cssCode);
      pass.push({ ok: !!(okFamily && okColor && hasVisitedRule), text: "Task 2: footer links styled incl. visited" });
    } catch (_) {
      pass.push({ ok: false, text: "Task 2: Error checking styles" });
    }

    // Task 3: first-letter styled (validate by code presence)
    try {
      ...
      pass.push({ ok: !!(hasSelector && hasSize && hasBg), text: "Task 3: first-letter styled" });
    } catch (_) {
      pass.push({ ok: false, text: "Task 3: Error checking styles" });
    }

    // Task 4: list style type lower-alpha
    try {
      ...
      pass.push({ ok: !!ok, text: "Task 4: submenu uses lower-alpha" });
    } catch (_) {
      pass.push({ ok: false, text: "Task 4: Error checking styles" });
    }

    // Task 5: hover scale rule present
    try {
      ...
      pass.push({ ok: !!(hasHover && hasScale && hasTransition), text: "Task 5: hover transform + transition" });
    } catch (_) {
      pass.push({ ok: false, text: "Task 5: Error checking styles" });
    }
...
function useHtmlCssLinkPlayground() {
  const previewRef = useRef(null);
...
<html>
  ...
  const linkRegex = useMemo(() => /<link[^>]*rel=["']stylesheet["'][^>]*href=["']styles\.css["'][^>]*>/i, []);
  ...
  useEffect(() => {
    ...
    previewRef.current.innerHTML = html;
  }, [htmlCode, cssCode, applyVersion, linkRegex]);
  ...
  function validate() {
    ...
    try {
      const okLink = linkRegex.test(htmlCode);
      pass.push({ ok: !!okLink, text: "Task: <link rel=stylesheet href=\"styles.css\"> is present" });
    } catch (_) {
      pass.push({ ok: false, text: "Task: Error checking <link> tag" });
    }

    // Check style actually applied to #title (green #16a34a)
    try {
      const el = container.querySelector("#title");
      const cs = el ? window.getComputedStyle(el) : null;
      const ok = cs && cs.color === "rgb(22, 163, 74)"; // #16a34a
      pass.push({ ok: !!ok, text: "Task: #title color is green from styles.css" });
    } catch (_) {
      pass.push({ ok: false, text: "Task: Error checking applied styles" });
    }
...
function Playground() {
  ...
  const highlighted = useMemo(() => highlightCss(cssCode), [cssCode]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
        <div className="p-3 border-b lg:border-b-0 lg:border-r border-zinc-200/60 dark:border-zinc-800">
          <div className="flex items-center justify-between mb-2">
            <div className="font-semibold text-sm">CSS editor</div>
          ...
          <div className="text-xs text-zinc-500 mt-2">
            Tip: Použijte přesné hodnoty v zadání pro vyhodnocení.
          </div>
        </div>
        <div className="p-3">
...
function LinkingPlayground() {
  const {
    ...
    validate,
    results,
  } = useHtmlCssLinkPlayground();

  const textRefCss = useRef(null);
  const preRefCss = useRef(null);
  const highlightedCss = useMemo(() => highlightCss(cssCode), [cssCode]);

  function syncScrollCss() {
    if (!textRefCss.current || !preRefCss.current) return;
    preRefCss.current.scrollTop = textRefCss.current.scrollTop;
    preRefCss.current.scrollLeft = textRefCss.current.scrollLeft;
  }
  ...
  return (
    <div className="rounded-2xl border border-zinc-200/60 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/60 shadow overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
        <div className="p-3 border-b lg:border-b-0 lg:border-r border-zinc-200/60 dark:border-zinc-800">
          <div className="flex items-center justify-between mb-2">
            <div className="font-semibold text-sm">HTML (head + body)</div>
            <label className="text-xs flex items-center gap-1">
              <input type="checkbox" checked={autoApply} onChange={(e) => setAutoApply(e.target.checked)} />
              Auto apply
            </label>
          ...
          <textarea
            ...
            spellCheck={false}
            className="w-full min-h-[200px] max-h-[260px] font-mono text-xs p-3 rounded-xl border border-zinc-200/60 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/60 focus:outline-none"
          />
          <div className="mt-2">
            <div className="font-semibold text-sm mb-2">styles.css</div>
            <div className="relative rounded-xl border border-zinc-200/60 dark:border-zinc-800 overflow-hidden">
              <pre
                ref={preRefCss}
                aria-hidden
                className="pointer-events-none whitespace-pre-wrap font-mono text-xs p-3 text-zinc-800 dark:text-zinc-200 bg-white/70 dark:bg-zinc-900/60 min-h-[200px] max-h-[260px] overflow-auto"
