...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      if (!username) {
        const rows = await sql(`select username, test1, test2, test3, test4, assignment_task_checked, assignment_midterm_ok, assignment_partner, assignment_final_points, auth_code from progress`);
        return res.status(200).json({ items: rows });
      }
      const rows = await sql(`select username, test1, test2, test3, test4, assignment_task_checked, assignment_midterm_ok, assignment_partner, assignment_final_points, auth_code from progress where username = $1`, [username]);
  ...
  if (req.method === 'POST') {
    try {
      ...
      await sql(`
        insert into progress (username, test1, test2, test3, test4, assignment_task_checked, assignment_midterm_ok, assignment_partner, assignment_final_points, auth_code)
