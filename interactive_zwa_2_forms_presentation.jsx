      ...
      <div className="space-y-4">
            ...
            <div className="space-y-2">
              <FieldRow label={<label htmlFor="text">label + input[type=text]</label>}>
                <input
                  id="text"
                  type="text"
                  className="w-full rounded border p-2 bg-white dark:bg-zinc-900"
                  {...common}
              ...
              <FieldRow label={<label htmlFor="textarea">textarea</label>}>
                <textarea
                  id="textarea"
                  className="w-full min-h-[80px] rounded border p-2 bg-white dark:bg-zinc-900"
                  {...common}
              ...
              <div className="pt-2">
                <button className="px-3 py-1.5 rounded bg-emerald-600 text-white">
                  Submit (local)
                </button>
              </div>
            </div>
              ...
              >
                <>
                  <input
                    ...
                    className="w-full rounded border p-2 bg-white dark:bg-zinc-900"
                    {...common}
                    value={values.datalistValue}
                    onChange={(e) => onChange('datalistValue', e.target.value)}
                  />
        ...
        <details className="rounded-xl border border-zinc-200/60 dark:border-zinc-800 bg-white/60 dark:bg-zinc-900/60 p-3">
          ...
          <div className="text-xs text-zinc-600 dark:text-zinc-400 mb-2">
            Akce ukáže, jaká data došla na server (může vyžadovat povolení vyskakovacích oken).
          </div>
          <form
            action="https://zwa.toad.cz/~xklima/vypisform.php"
            method="post"
            target="_blank"
        ...
        <details className="rounded-xl border border-zinc-200/60 dark:border-zinc-800 bg-white/60 dark:bg-zinc-900/60 p-3">
          ...
          <div className="text-xs text-zinc-600 dark:text-zinc-400 mb-2">
            Zadejte HTML vašeho formuláře, spustíme lokální kontroly a volitelně i online validaci.
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setFlags((f) => ({ ...f, readOnly: e.target.checked }))}
              />
              <span>readonly</span>
            </label>
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setFlags((f) => ({ ...f, disabled: e.target.checked }))}
              />
              <span>disabled</span>
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={flags.requiredOn}
                onChange={(e) => setFlags((f) => ({ ...f, requiredOn: e.target.checked }))}
              />
              <span>required</span>
            </label>
            <div className="flex items-center gap-2">
              ...
              <select
                className="rounded border p-1.5 bg-white dark:bg-zinc-900"
                value={flags.autocomplete}
                onChange={(e) => setFlags((f) => ({ ...f, autocomplete: e.target.value }))}
              >
            ...
            <div className="flex items-center gap-2 col-span-2">
              ...
              <select
                className="rounded border p-1.5 bg-white dark:bg-zinc-900"
                value={flags.autofocus}
                onChange={(e) => setFlags((f) => ({ ...f, autofocus: e.target.value }))}
              >
            ...
            <div className="flex items-center gap-2 col-span-2">
              ...
              <input
                className="flex-1 rounded border p-1.5 bg-white dark:bg-zinc-900"
                value={flags.patternTel}
                onChange={(e) => setFlags((f) => ({ ...f, patternTel: e.target.value }))}
              />
            </div>
          </div>
        </SectionCard>
        ...
        <div className="mt-2 text-xs">
          <div className={lc.passed ? 'text-emerald-600' : 'text-amber-600'}>
            {lc.passed
              ...
              : `Nalezeno ${lc.issues.length} připomínek:`}
          </div>
          {!lc.passed && (
      ...
      <div className="flex flex-col">
        {!showPreview && (
          <div className="text-xs text-zinc-500 mb-1">
            Pozn.: Náhled se nezobrazuje – editor je zaměřen na strukturu a validitu.
          </div>
        )}
        {showPreview ? (
          <SandboxedPreview
          ...
          <pre className="rounded border p-2 bg-white/70 dark:bg-zinc-900/60 text-xs whitespace-pre-wrap">
            {html}
          </pre>
        )}
        {results?.type === 'online' && (
...
function FormsSections() {
  const tplStandard = [
    ...
    '  <head>',
    '    <meta charset="utf-8">',
    '    <title>Standard Inputs</title>',
    '  </head>',
    '  <body>',
    '    <!-- Úkol: vytvořte form s label+for, input[type=text], textarea, select, button -->',
    '  </body>',
    '</html>',
  ].join('\n');
  const exStandard = [
    ...
    '  <head>',
    '    <meta charset="utf-8">',
    '    <title>Standard Inputs (Example)</title>',
    '  </head>',
    '  <body>',
    '    <form>',
  ...
  const checkStandard = (html) => {
    const issues = [];
    const must = (re, msg) => {
      if (!re.test(html)) issues.push(msg);
    };
    must(/<label[^>]+for=/i, 'Alespoň jeden <label for=...>');
    must(/<input[^>]+type=\"text\"/i, 'Alespoň jeden input[type=text]');
    must(/<textarea/i, 'Přidejte <textarea>');
    must(/<select/i, 'Přidejte <select>');
    return { issues, passed: issues.length === 0 };
  };
  ...
  const tplGrouping = [
    ...
    '  </head>',
    '  <body>',
    '    <!-- Úkol: vytvořte form a seskupte prvky do fieldset + legend, alespoň 1 label+input -->',
    '  </body>',
    '</html>',
  ].join('\n');
  const exGrouping = [
  ...
  const checkGrouping = (html) => {
    const issues = [];
    const must = (re, msg) => {
      if (!re.test(html)) issues.push(msg);
    };
    must(/<fieldset/i, 'Použijte <fieldset>');
    must(/<legend/i, 'Použijte <legend>');
    must(/<label[^>]+for=/i, 'Spárujte label s inputem');
    return { issues, passed: issues.length === 0 };
  };
  ...
  const tplAttributes = [
    '<!doctype html>',
    '<html>',
    '  <head><meta charset="utf-8"><title>Attributes</title></head>',
    '  <body>',
    '    <!-- Úkol: přidejte form s inputy, použijte readonly, required, autocomplete(on/off), autofocus a disabled button -->',
    '  </body>',
    '</html>',
  ].join('\n');
  const exAttributes = [
    '<!doctype html>',
    '<html>',
    '  <head><meta charset="utf-8"><title>Attributes (Example)</title></head>',
    '  <body>',
    '    <form autocomplete="on">',
    '      <input type="text" name="immutable" value="jen pro čtení" readonly>',
  ...
  const checkAttributes = (html) => {
    const issues = [];
    if (!/readonly/i.test(html)) issues.push('Přidejte readonly');
    if (!/required/i.test(html)) issues.push('Přidejte required');
    if (!/autocomplete=\"(on|off)\"/i.test(html)) issues.push('Nastavte autocomplete');
    return { issues, passed: issues.length === 0 };
  };

  const tplInputs = [
    '<!doctype html>',
    '<html><head><meta charset="utf-8"><title>HTML5 inputs</title></head><body>',
    '  <!-- Úkol: vytvořte form s email, url, tel(+pattern), date, color, range(min/max/step) -->',
    '</body></html>',
  ].join('\n');
  const exInputs = [
    '<!doctype html>',
    '<html><head><meta charset="utf-8"><title>HTML5 inputs (Example)</title></head><body>',
    '  <form>',
  ...
  const checkInputs = (html) => {
    const issues = [];
    ['email', 'url', 'tel', 'date', 'color', 'range'].forEach((t) => {
      if (!new RegExp(`type=\\\"${t}\\\"`, 'i').test(html)) issues.push(`Chybí input[type=${t}]`);
    });
    if (/type=\"tel\"/i.test(html) && !/pattern=/i.test(html))
      issues.push('Pro telefon nastavte pattern');
    return { issues, passed: issues.length === 0 };
  };

  const tplMeter = [
    '<!doctype html>',
    '<html><head><meta charset="utf-8"><title>Meter/Progress</title></head><body>',
    '  <!-- Úkol: přidejte meter(min,max,low,high,optimum,value) a progress(max,value) -->',
    '</body></html>',
  ].join('\n');
  const exMeter = [
    '<!doctype html>',
    '<html><head><meta charset="utf-8"><title>Meter/Progress (Example)</title></head><body>',
    '  <meter min="0" max="100" low="30" high="80" optimum="60" value="42"></meter>',
    '  <progress max="100" value="42"></progress>',
    '</body></html>',
  ].join('\n');
  const checkMeter = (html) => {
    const issues = [];
    if (!/<meter/i.test(html)) issues.push('Přidejte <meter>');
    if (!/<progress/i.test(html)) issues.push('Přidejte <progress>');
    return { issues, passed: issues.length === 0 };
  };

  const tplDatalist = [
    '<!doctype html>',
    '<html><head><meta charset="utf-8"><title>Datalist</title></head><body>',
    '  <!-- Úkol: input s atributem list a datalist s minimálně 2 options -->',
    '</body></html>',
  ].join('\n');
  const exDatalist = [
    '<!doctype html>',
    '<html><head><meta charset="utf-8"><title>Datalist (Example)</title></head><body>',
  ...
  const checkDatalist = (html) => {
    const issues = [];
    if (!/list=\"[\w-]+\"/i.test(html)) issues.push('Input musí mít list=...');
    if (!/<datalist/i.test(html)) issues.push('Chybí <datalist>');
    return { issues, passed: issues.length === 0 };
  };
  ...
  function TryStandard() {
    ...
    return (
      <form className="space-y-2">
        <div className="grid grid-cols-1 sm:grid-cols-[120px,1fr] gap-2 items-center">
          <label htmlFor="name">Jméno</label>
          <input
            id="name"
            className="rounded border p-2 bg-white dark:bg-zinc-900"
            value={name}
    ...
    return (
      <form className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <input
          ...
          readOnly
        />
        <button className="rounded bg-zinc-200 p-2" disabled>
          Disabled
        </button>
      </form>
    );
    ...
    return (
      <form className="grid grid-cols-1 md:grid-cols-2 gap-2 items-center">
        ...
        <input
          ...
          placeholder="+420 123 456 789"
        />
        <input type="date" className="rounded border p-2 bg-white dark:bg-zinc-900" />
        <input type="color" />
        <div className="flex items-center gap-2">
          <input
            type="range"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
        ...
        <div className="space-y-2">
          <meter
            ...
            className="w-full"
          ></meter>
          <progress max="100" value={val} className="w-full"></progress>
        </div>
      </div>
    );
  }
  function TryDatalist() {
    return (
      <div>
        <input
          list="cities-try"
  ...
  return (
    <div className="space-y-4">
        ...
        theory={
          <>
            <p>
              ...
              <Code>button</Code>. Při odeslání prohlížeč vytvoří dvojice <em>name=value</em> pro
              každý vyplněný, nepodmínečně povolený prvek a odešle je na server (metodou GET nebo
              POST).
            </p>
            <p className="mt-2">
              Klíčová je vazba <Code>label</Code> → <Code>input</Code> pomocí atributů
              <Code>for</Code> (na <Code>label</Code>) a <Code>id</Code> (na cílovém prvku). Zvyšuje
              to přístupnost (screen readery oznámí správný popisek) a použitelnost (klik na popisek
      ...
      <Block
        id="grouping"
        title="Seskupení (fieldset/legend)"
        theory={
          <>
            <p>
              <Code>fieldset</Code> umožňuje logicky seskupit související ovládací prvky (např.
              kontaktní údaje) a <Code>legend</Code> poskytuje textový titulek pro celou skupinu.
              Tato sémantika významně pomáhá přístupnosti: čtečky obrazovky oznámí název skupiny
              před čtením jejích polí a uživatel se tak lépe orientuje.
            </p>
            <p className="mt-2">
              Můžete mít více skupin ve formuláři; myslete na to, aby každá skupina měla jasný účel
              a aby prvky ve skupině sdílely společný kontext (např. doručovací adresa vs.
              fakturační adresa). Vyhnete se tak zmatku i duplicitám.
            </p>
          </>
        }
        example={exGrouping}
        Try={TryGrouping}
        validate={{ initial: tplGrouping, check: checkGrouping }}
      />

      <Block
        id="attributes"
        title="Atributy (readonly, disabled, autocomplete, autofocus, required)"
        theory={
          <>
            <p>
              <Code>readonly</Code> brání úpravě hodnoty, ale pole je stále odesíláno. Naproti tomu
              <Code>disabled</Code> pole vyřadí z tab pořadí a NEODESÍLÁ se – to je důležité pro
              logiku na serveru. Používejte jej například pro dočasně neaktivní volby.
            </p>
            <p className="mt-2">
              <Code>required</Code> vynucuje vyplnění, prohlížeč blokuje odeslání a zobrazí
              nápovědu.
              <Code>autocomplete</Code> (<Code>on</Code>/<Code>off</Code>) řídí návrhy prohlížeče a
              může urychlit vyplňování. <Code>autofocus</Code> zajistí zaostření po načtení – mělo
              by se používat střídmě a ideálně jen jednou na stránce.
            </p>
          </>
        }
        example={exAttributes}
        Try={TryAttributes}
        validate={{ initial: tplAttributes, check: checkAttributes }}
      />

      <Block
        id="inputs"
        title="HTML5 typy inputů"
        theory={
          <>
            <p>
              HTML5 přineslo specializované typy jako <Code>email</Code>, <Code>url</Code>,
              <Code>tel</Code>, <Code>date</Code>, <Code>color</Code>, <Code>range</Code> apod.
              Prohlížeč pak poskytuje správnou klávesnici na mobilech, základní validaci a lepší UX
              bez JavaScriptu. Např. <Code>email</Code> kontroluje formát a <Code>range</Code>{' '}
              dovoluje nastavovat číselnou hodnotu posuvníkem.
            </p>
            <p className="mt-2">
              U číselných a rozsahových vstupů dodávejte <Code>min</Code>, <Code>max</Code>,
              případně
              <Code>step</Code>. Pro <Code>tel</Code> je vhodné doplnit <Code>pattern</Code> kvůli
              validaci formátu. Klientská validace je pouze první linie – server musí pravidla
              zopakovat nezávisle, jinak hrozí obcházení omezení.
            </p>
          </>
        }
        example={exInputs}
        Try={TryInputs}
        validate={{ initial: tplInputs, check: checkInputs }}
      />

      <Block
        id="meter"
        title="Meter a Progress"
        theory={
          <>
            <p>
              <Code>meter</Code> reprezentuje měření známé veličiny v definovaném rozsahu (např.
              využití baterie). Atributy <Code>low</Code>, <Code>high</Code> a <Code>optimum</Code>{' '}
              dávají prohlížeči kontext, kdy je hodnota v normě či mimo ni. Není vhodné pro průběh
              úlohy.
            </p>
            <p className="mt-2">
        ...
        example={exMeter}
        Try={TryMeter}
        validate={{ initial: tplMeter, check: checkMeter }}
      />
      ...
      <Block
        ...
        theory={
          <>
            <p>
              ...
              <Code>list</Code>. Uživatel může napsat libovolnou hodnotu, ale dostane nápovědu. Není
              to totéž co <Code>select</Code> – neomezuje vstup pouze na nabídku.
            </p>
            <p className="mt-2">
        ...
        example={exDatalist}
        Try={TryDatalist}
        validate={{ initial: tplDatalist, check: checkDatalist }}
      />
    </div>
  );
}
